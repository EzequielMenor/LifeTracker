...
export function SettingsView({ data, onUpdate, onClose }: SettingsProps) {
	...
	const handleReset = () => {
		if (confirm('¿Estás seguro de reiniciar todo tu progreso (Nivel 1, 0 XP)? Esta acción no se puede deshacer.')) {
			resetProgress();
		}
	};

	// Local state for form editing before saving
	const [habits, setHabits] = React.useState<{ id: string; name: string; attribute: AttributeKey }[]>(
		Array.isArray(data.config?.habits) && typeof data.config.habits[0] === 'string'
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-2 gap-4">
						<div className="space-y-2">
							<label className="text-sm font-medium">Meta de Sueño (horas)</label>
							<input
								type="number"
								step="0.5"
						...
						<div className="space-y-2">
							<label className="text-sm font-medium">Límite de Móvil (horas)</label>
							<input
								type="number"
								step="0.5"
								className="w-full p-2 bg-muted rounded-md border border-transparent focus:border-primary outline-none"
			...
			<Card>
				...
				<CardContent className="space-y-4">
					...
					<div className="space-y-2">
						...
						{habits.length === 0 && (
							<p className="text-center text-muted-foreground py-4 text-sm italic">No tienes hábitos definidos. ¡Añade uno arriba!</p>
						)}
					</div>
				</CardContent>
			</Card>
